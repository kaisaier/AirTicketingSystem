		          ...
		          "bSort" : true,//设置自动排序的地方,true表示排序，false表示取消自动排序
			            ...
			            "sSearch" : "搜索" 
							...
							"fnCreatedRow": function (nRow, aData, iDataIndex) {
								var URL1 = "../servlet/AirInfoFuKuanServlet"
								var URL2 = "../servlet/OrderDeleteServlet";
								var airid = $('td:eq(0)', nRow).text();
								var chakan = $('td:eq(0)', nRow).text();
								var shanchu = $('td:eq(0)', nRow).text();
								var fukuanway = $('td:eq(5)', nRow).text();
								if(fukuanway === '已付款')
								$('td:eq(6)', nRow).html("<a href='../servlet/OrderChakanServlet?chakan="+chakan+"'>查看</a>");
								if(fukuanway === '待付款')
									$('td:eq(6)', nRow).html("<a href=javascript:void(0) onclick=if(window.confirm('是否确定付款？'))this.href='"+URL1+"?airid="+airid+"'>付款</a>");
								if(fukuanway === '已过期')
									$('td:eq(6)', nRow).html("<a href=javascript:void(0) onclick=if(window.confirm('是否确定删除？'))this.href='"+URL2+"?shanchu="+shanchu+"'>删除</a>");
							}
						});
